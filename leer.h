...
List<T> leer<T>::extraerNotas(const char *x){
  ...
  List<string> listaNotas;
    ...
    while (measure) {
      ...
      while (nota) { // Iterar sobre las notas

        XMLElement *rest = nota->FirstChildElement("rest");  // Asegurarse de que no sea un silencio (rest)
        if (nota != rest) {  // Extraer la información de la nota
          ...
          if (pitch) {
            ...
            if (step) {
              ...
              // Verificar si la nota tiene una alter
              if (alter) {
                ...
                if (alterS == "1") {
                  stepS += "#"; // Sostenido
                    ...
                    stepS = bemolToSostenido[stepS];  // Convertir bemol a su equivalente sostenido
                }
              }// almcacenar nota
